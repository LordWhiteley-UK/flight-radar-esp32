...
|  Port 80, STA (LAN) only  |        | 22s polling w/ OAuth2     |
...
| `ui_Screen2` | WiFi picker (used on first boot when no credentials; no softAP).       |
...
| `/upload` | POST   | JSON body `{clientId, clientSecret}`; saved to NVS, then restart.      |
| `/clear`  | POST   | Forgets the OpenSky credentials (`platform_forget_opensky_creds`), restarts. |

There is **no softAP** (verified 2026-09-08 — earlier docs described a
`FlightRadar-Setup` AP that never existed). WiFi is configured on-screen via
Screen2, then — if OpenSky credentials are missing — the webserver starts on
the STA interface, reachable at `http://<board-ip>/` (IP shown on the radar
screen).
